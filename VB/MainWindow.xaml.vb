        ...
        Private Shared Sub ApplyStyles(ByVal doc As Document, ByVal _table As Table, ByVal header_Row As TableRow)
            ...
            For Each tr As TableRow In _table.Rows
                If Not tr.IsFirst Then
                    ...
                    doc.Paragraphs.Get(tr.FirstCell.Range.Start).Style = doc.ParagraphStyles("MyTableCaptionStyle")
                End If
            Next
        End Sub
...
'#End Region  ' #applyborders
        Private Sub btnInsertTable_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
            ...
            Dim cmd As InsertTableColumnToTheLeftCommand = New InsertTableColumnToTheLeftCommand(Me.richEditControl1)
            cmd.Execute()
            ...
            ' Finalize formatting
            Call ApplyBorders(_table)
            Call ApplyStyles(doc, _table, header_Row)
...
'#Region "#celldelegate"
        ...
        Private Sub MakeMultiplicationCell(ByVal cell As TableCell, ByVal i As Integer, ByVal j As Integer)
            Me.richEditControl1.Document.InsertText(cell.Range.Start, String.Format("{0}*{1} is {2}", i + 2, j + 2, (i + 2) * (j + 2)))
        End Sub
...
'#Region "#insertcolumnnumbers"
        Private Sub InsertColumnNumbers(ByVal row As TableRow)
            For Each _cell As TableCell In row.Cells
                If _cell.Index = 0 Then Continue For
                _cell.BackgroundColor = Color.Empty
